...
thermalDini <- list(list(beta = c(0.9, 0.002, 0.03, 1.59), phil = 0.12),
                    list(beta = c(0.9, 0.002, 0.03, 1.59)/2, phil = 0.12/2))

thermalD_bayesian_noclust <- run.jags(model = "ThermalC_bayesian.bug",
                              monitor=c("beta", "phil"),
...
###The posterior summary checks?
plot(thermalD_bayesian_noclust, "trace", vars = c("beta", "phil"))
plot(thermalD_bayesian_noclust, "autocorr", vars = c("beta", "phil"))
...
thermalDini <- list(list(beta = c(0.9, 0.002, 0.03, 1.59), phi = 0.12,sigma_y=0.12, 
                         sigma.b = 0.12),
                    list(beta = c(0.9, 0.002, 0.03, 1.59)/2, phi = 0.12/2,sigma_y=0.12/2, 
                         sigma.b = 0.12/2))

thermalDclust_bayesian <- run.jags(model = "ThermalC_bayesian.bug",
                                   monitor=c("beta", "phi", "icc"),
                                   data = thermalDdata, inits = thermalDini, n.chains = 2,
                                   sample = 30000, burnin = 20000,
                                   thin = 15, adapt = 40000,
                                   method = "rjags", jags.refresh = 10)
###The posterior summary checks?
plot(thermalDclust_bayesian, "trace", vars = c("beta", "phi","icc"))
plot(thermalDclust_bayesian, "autocorr", vars = c("beta", "phi","icc"))

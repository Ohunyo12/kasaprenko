...
toplot_thermalc_noclust %>% ggplot(aes(x = c("Intercept", "Elevation", "Temperature", "Moist Content"), y = Mean))  +
...
```{r Thermal.D, cache=T}
##The Beta regression approach is to reparameterize in terms of mu and phi where mu will be equivalent to y_hat that we predict.In this parameterization you will have alpha=mu*phil and Beta=(1-mu)*phil. You can model mu as the logit of the linear combination
#Bayesian Linear Regression (without clustering and heteroscedasticity)
#normal assumption for Thermal.C
cat("
    model{
...
thermalDdata <- list(N = nrow(alex_data1),
                     ...
                     MC = alex_data1$M.C,
                     samp = rep(0,nrow(alex_data1))
)

thermalDini <- list(list(beta = c(-3.4, -0.005, 0.19, -0.84), phi = 1/exp(-1.13)^2),
                    list(beta = c(-3.4, -0.005, 0.19, -0.84)/2, phi = (1/exp(-1.13)^2)/2))

thermalD_bayesian_noclust <- run.jags(model = "ThermalD_bayesian.bug",
                              monitor=c("beta", "var_y", "res_y", "sigma_y", "phi", "OR"),
                              data = thermalDdata, inits = thermalDini, n.chains = 2,
                              sample = 5000, burnin = 20000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)
#thermalD_bayesian_noclust


###The posterior summary checks?
plot(thermalD_bayesian_noclust, "trace", vars = c("beta", "phi", "var_y"))
plot(thermalD_bayesian_noclust, "autocorr", vars = c("beta", "phi", "var_y"))
dic_thermalD_noclust <- extract(thermalD_bayesian_noclust, what = "dic")
dic_thermalD_noclust
```

```{r}
knitr::kable(thermalD_bayesian_noclust$summaries[1:5, c(1:5)])
```
...
```{r}
knitr::kable(thermalC_bayesian_clust$summaries[1:7, c(1:5)])
...
```{r}
knitr::kable(SHclust_bayesian$summaries[1:7, c(1:5)])
```

### Thermal.D

```{r Thermal.D2, cache=T}
##The Beta regression approach is to reparameterize in terms of mu and phi where mu will be equivalent to y_hat that we predict.In this parameterization you will have alpha=mu*phi and Be=(1-mu)*phi. You can model mu as the logit of the linear combination
#Bayesian Linear Regression (without clustering and heteroscedasticity)
#normal assumption for Thermal.D
cat("
    model{
    #### #the Beta likelihood for the data
    for(i in 1: N){
    ...
    logit(mu[i]) <- beta[2]*Elevation[i] + beta[3]*Temp[i] + beta[4]*MC[i] + b[Id[i]]
...
thermalDdata <- list(N = nrow(alex_data1),
                     ...
                     Temp = alex_data1$Temp,
                     MC = alex_data1$M.C,
                     J = length(unique(alex_data1$Id)), 
                     samp = rep(0, nrow(alex_data1))         
                     )
                                
                     
thermalDini <- list(list(beta = c(-3.4, -0.005, 0.19, -0.84), phi = 1/exp(-1.13), 
                         sigma.b = 0.12),
                    list(beta =  c(-3.4, -0.005, 0.19, -0.84)/2, phi = (1/exp(-1.13))/2, 
                         sigma.b = 0.12/2))

thermalDclust_bayesian <- run.jags(model = "ThermalD_bayesian.bug",
                                   monitor=c("beta", "phi", "icc", "sigma.b", "OR"),
                                   data = thermalDdata, inits = thermalDini, n.chains = 2,
                                   sample = 10000, burnin = 20000,
                                   thin = 15, adapt = 40000,
                                   method = "rjags", jags.refresh = 10)
###The posterior summary checks?
plot(thermalDclust_bayesian, "trace", vars = c("beta", "phi","icc", "sigma.b"))
plot(thermalDclust_bayesian, "autocorr", vars = c("beta", "phi","icc", "sigma.b"))
dic_thermalDclust <- extract(thermalDclust_bayesian, what = "dic")
dic_thermalDclust
...
```{r}
knitr::kable(thermalDclust_bayesian$summaries[1:6, c(1:5)])
```
...
```{r}
knitr::kable(thermalC_bayesian_sp$summaries[1:8, c(1:5)])
toplot_thermalc_spatial <- as.data.frame(thermalC_bayesian_sp$summaries[1:4, c(1:5)])

```


### Variogram for S.H Based on Bayesian-Linear Regression Model
```{r }
SH_res <- SH_bayesian$summaries[str_detect(row.names(SH_bayesian$summaries), "res"), "Mean"]
SH_gdata <- as.geodata(cbind(alex_data$Longitude, alex_data$latitude, SH_res))

### varogram across all clusters (we can just keep this)
dd_matrix <- as.matrix(dist(alex_data[, 2:3], diag = T, upper = T))
vvs_SH <- variog(SH_gdata, uvec = quantile(unique(as.numeric(dd_matrix)), seq(0, 1, 0.01)), option = "bin")
plot(vvs_SH, type="p", pch=19, col = 2,cex.lab = 1.3,
             main = "", 
             ylab = expression(v(u)), lwd = 2 )
...
SHini <- list(list(beta = c(0.9, 0.002, 0.03, 1.59), sigma_y=0.12, 
                         tetha1 = 0.5, phi = 1.0, tetha12 = 1.23),
                    list(beta = c(0.9, 0.002, 0.03, 1.59)/2, sigma_y=0.12/2, 
                         tetha1 = 0.5/2, phi = 1.0/2, tetha12 = 1.23/2)
                    )

SH_bayesian_sp <- run.jags(model =  "thermal_spbayesian.bug",
                              monitor=c("beta", "sigma_y", "tetha1", "tetha12", "phi"),
                              data = SHdata, inits = SHini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)
plot(SH_bayesian_sp, "trace", vars = c("beta", "sigma_y", "tetha1", "tetha12", "phi"))
plot(SH_bayesian_sp, "autocorr", vars = c("beta", "sigma_y", "tetha1", "tetha12", "phi"))
dic_SH_sp <- extract(SH_bayesian_sp, what = "dic")
dic_SH_sp
```

```{r}
knitr::kable(SH_bayesian_sp$summaries[1:8, c(1:5)])
```
...
```{r}
knitr::kable(thermalC_bayesian_spclust$summaries[1:11, c(1:5)])
...
#NOTE: All covariates are not significant because the credible interval contains 0
  
```

### Variogram for SH Based on Clustered-Data Model

```{r}
SH_res <- SHclust_bayesian$summaries[str_detect(row.names(SHclust_bayesian$summaries), "res"), "Mean"]
SH_gdata <- as.geodata(cbind(alex_data$Longitude, alex_data$latitude, SH_res))
...
SH_bayesian_spclust <- run.jags(model =  "thermal_spclustbayesian.bug",
                              monitor=c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"),
                              data = SHdata, inits = SHini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
...
###The posterior summary checks?
plot(SH_bayesian_spclust, "trace", vars = c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"))
plot(SH_bayesian_spclust, "autocorr", vars = c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"))

dic_SH_spclust <- extract(SH_bayesian_spclust, what = "dic")
dic_SH_spclust
```


```{r}
knitr::kable(SH_bayesian_spclust$summaries[1:11, c(1:5)])
```

## Bayesian Linear Regression (without cluster 4,5 and heteroscedasticity)

### Thermal.C


```{r Thermal.C5, cache=T}
###normal assumption for Thermal.C
alex_data2=alex_data[!alex_data$Id%in%c(4,5),] #We filtered out cluster 4 and 5
cat("
    model{
    #### #the normal likelihood for the data
    for(i in 1: N) {
...
thermalC2_bayesian_noclust <- run.jags(model = "ThermalC_bayesian.bug",
                              monitor=c("beta", "sigma_y","res"),
                              data = thermalCdata, inits = thermalCini, n.chains = 2,
                              sample = 30000, burnin = 20000,
                              thin = 15, adapt = 40000,
...
###The posterior summary checks?
plot(thermalC2_bayesian_noclust, "trace", vars = c("beta", "sigma_y"))
plot(thermalC2_bayesian_noclust, "autocorr", vars = c("beta", "sigma_y"))
dic_thermalC2_noclust <- extract(thermalC2_bayesian_noclust, what = "dic")
dic_thermalC2_noclust
```



```{r}
knitr::kable(thermalC2_bayesian_noclust$summaries[1:5, c(1:5)])
```
...
## Spatial Model without cluster 4 and 5 data

### Variogram for Thermal C Based on Bayesian-Linear Regression Model

```{r }
thermalC_res1 <- thermalC2_bayesian_noclust$summaries[str_detect(row.names(thermalC2_bayesian_noclust$summaries), "res"), "Mean"]
thermalC_gdata <- as.geodata(cbind(alex_data2$Longitude, alex_data2$latitude, thermalC_res1))
...
thermalC2_bayesian_sp <- run.jags(model =  "thermal_spbayesian.bug",
                              monitor=c("beta", "sigma_y", "tetha1", "tetha12", "phi"),
                              data = thermalCdata, inits = thermalCini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)
thermalC2_bayesian_sp
plot(thermalC2_bayesian_sp, "trace", vars = c("beta", "sigma_y", "tetha1", "tetha12", "phi"))
plot(thermalC2_bayesian_sp, "autocorr", vars = c("beta", "sigma_y", "tetha1", "tetha12", "phi"))
dic_thermalC2_sp <- extract(thermalC2_bayesian_sp, what = "dic")
dic_thermalC2_sp 
```


```{r}
knitr::kable(thermalC2_bayesian_sp$summaries[1:8, c(1:5)])
```

##CLUSTER MODEL  
##Thermal.C  without 4 and 5 clusters
...
thermalC2data <- list(N = nrow(alex_data2),
                     Id = alex_data2$Id,
                     y = alex_data2$Thermal.C,
                     Elevation = alex_data2$Elevation,
                     Temp = alex_data2$Temp,
...
thermalC2ini <- list(list(beta = c(0.9, 0.002, 0.03, 1.59), sigma_y=0.12, 
                         sigma.b = 0.12),
                    list(beta = c(0.9, 0.002, 0.03, 1.59)/2, sigma_y=0.12/2, 
                         sigma.b = 0.12/2))

thermalC2W_bayesian_clust <- run.jags(model =  "ThermalC_bayesian.bug",
                              monitor=c("beta", "sigma_y", "sigma.b","icc","res"),
                              data = thermalC2data, inits = thermalC2ini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)
...
###The posterior summary checks?
plot(thermalC2W_bayesian_clust, "trace", vars = c("beta", "sigma_y","sigma.b","icc"))
plot(thermalC2W_bayesian_clust, "autocorr", vars = c("beta", "sigma_y","sigma.b","icc"))
dic_thermalC2W_clust <- extract(thermalC2W_bayesian_clust, what = "dic")
dic_thermalC2W_clust
```

```{r}
knitr::kable(thermalC2W_bayesian_clust$summaries[1:7, c(1:5)])
```
...
```{r}
thermalC_res <- thermalC2W_bayesian_clust$summaries[str_detect(row.names(thermalC2W_bayesian_clust$summaries), "res"), "Mean"]
thermalC2W_gdata <- as.geodata(cbind(alex_data2$Longitude, alex_data2$latitude, thermalC_res))

### varogram across all clusters (we can just keep this)
dd_matrix <- as.matrix(dist(alex_data[, 2:3], diag = T, upper = T))
vvs_thermalC2W <- variog(thermalC2W_gdata, uvec = quantile(unique(as.numeric(dd_matrix)), seq(0, 1, 0.01)), option = "bin")
plot(vvs_thermalC2W, type="p", pch=19, col = 2,cex.lab = 1.3,
             main = "", 
             ylab = expression(v(u)), lwd = 2 )

```

### Thermal C spatial cluster model without cluster 4 and 5 data
...
thermalC2W_bayesian_spclust <- run.jags(model =  "thermal_spclustbayesian.bug",
                              monitor=c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"),
                              data = thermalC2data, inits = thermalC2ini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
...
###The posterior summary checks?
plot(thermalC2W_bayesian_spclust, "trace", vars = c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"))
plot(thermalC2W_bayesian_spclust, "autocorr", vars = c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"))
dic_thermalC2W_spclust <- extract(thermalC2W_bayesian_spclust, what = "dic")
dic_thermalC2W_spclust

```


```{r}
knitr::kable(thermalC2W_bayesian_spclust$summaries[1:11, c(1:5)])
```

## Bayesian Linear Regression (without 4, 5 clusters and heteroscedasticity)
...
SH2_bayesian_noclust <- run.jags(model = "ThermalC_bayesian.bug",
                              monitor=c("beta", "sigma_y","res"),
                              data = SHdata, inits = SHini, n.chains = 2,
                              sample = 30000, burnin = 20000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)

###The posterior summary checks?
plot(SH2_bayesian_noclust, "trace", vars = c("beta", "sigma_y"))
plot(SH2_bayesian_noclust, "autocorr", vars = c("beta", "sigma_y"))
dic_SH2_noclust <- extract(SH2_bayesian_noclust, what = "dic")
dic_SH2_noclust
```
...
```{r}
knitr::kable(SH2_bayesian_noclust$summaries[1:5, c(1:5)])
```
...
```{r }
alex_data2=alex_data[!alex_data$Id%in%c(4,5),] #We filtered out cluster 4 and 5
SH_res1 <- SH2_bayesian_noclust$summaries[str_detect(row.names(SH2_bayesian_noclust$summaries), "res"), "Mean"]
SH_gdata <- as.geodata(cbind(alex_data2$Longitude, alex_data2$latitude, SH_res1))
...
SH2_bayesian_sp <- run.jags(model =  "thermal_spbayesian.bug",
                              monitor=c("beta", "sigma_y", "tetha1", "tetha12", "phi"),
                              data = SHdata, inits = SHini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
                              ...
                              method = "rjags", jags.refresh = 10)
SH2_bayesian_sp
plot(SH2_bayesian_sp, "trace", vars = c("beta", "sigma_y", "tetha1", "tetha12", "phi"))
plot(SH2_bayesian_sp, "autocorr", vars = c("beta", "sigma_y", "tetha1", "tetha12", "phi"))
dic_SH2_sp <- extract(SH2_bayesian_sp, what = "dic")
dic_SH2_sp 
```


```{r}
knitr::kable(SH2_bayesian_sp$summaries[1:8, c(1:5)])
```

### S.H  cluster model without cluster 4 and 5 data
...
SH2data <- list(N = nrow(alex_data2),
                     Id = alex_data2$Id,
                     y = alex_data2$S.H,
                     Elevation = alex_data2$Elevation,
                     Temp = alex_data2$Temp,
...
SH2ini <- list(list(beta = c(0.9, 0.002, 0.03, 1.59), sigma_y=0.12, 
                         sigma.b = 0.12),
                    list(beta = c(0.9, 0.002, 0.03, 1.59)/2, sigma_y=0.12/2, 
                         sigma.b = 0.12/2))

SH2W_bayesian_clust <- run.jags(model =  "ThermalC_bayesian.bug",
                              monitor=c("beta", "sigma_y", "sigma.b","icc","res"),
                              data = SH2data, inits = SH2ini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)
...
###The posterior summary checks?
plot(SH2W_bayesian_clust, "trace", vars = c("beta", "sigma_y","sigma.b","icc"))
plot(SH2W_bayesian_clust, "autocorr", vars = c("beta", "sigma_y","sigma.b","icc"))
dic_SH2W_clust <- extract(SH2W_bayesian_clust, what = "dic")
dic_SH2W_clust
```

```{r}
knitr::kable(SH2W_bayesian_clust$summaries[1:7, c(1:5)])
```

## Spatial-Cluster Model

### Variogram for S.H Based on Clustered-Data Model

```{r}
SH2W_res <- SH2W_bayesian_clust$summaries[str_detect(row.names(SH2W_bayesian_clust$summaries), "res"), "Mean"]
SH2W_gdata <- as.geodata(cbind(alex_data2$Longitude, alex_data2$latitude, SH2W_res))

### varogram across all clusters (we can just keep this)
dd_matrix <- as.matrix(dist(alex_data[, 2:3], diag = T, upper = T))
vvs_SH2W <- variog(SH2W_gdata, uvec = quantile(unique(as.numeric(dd_matrix)), seq(0, 1, 0.01)), option = "bin")
plot(vvs_SH2W, type="p", pch=19, col = 2,cex.lab = 1.3,
             main = "", 
             ylab = expression(v(u)), lwd = 2 )

```

### S.H without 4 and 5 clusters 
...
thermalC3Wdata <- list(N = nrow(alex_data2),
                     Id = alex_data2$Id,
                     y = alex_data2$S.H,
                     u = vvs_SH2W$u,
                     vu = vvs_SH2W$v,
                     uvec = vvs_SH2W$uvec,
                     N2 = length(vvs_SH2W$u),
                     Elevation = alex_data2$Elevation,
                     Temp = alex_data2$Temp,
                     MC = alex_data2$M.C,
                     J = length(unique(alex_data2$Id))
...
thermalC3Wini <- list(list(beta = c(0.9, 0.002, 0.03, 1.59), sigma_y=0.12, 
                         sigma.b = 0.12, tetha1 = 0.5, phi = 1.0, tetha12 = 1.23),
                    list(beta = c(0.9, 0.002, 0.03, 1.59)/2, sigma_y=0.12/2, 
                         sigma.b = 0.12/2 , tetha1 = 0.5/2, phi = 1.0/2, tetha12 = 1.23/2)
                    )

SH3W_bayesian_spclust <- run.jags(model =  "thermal_spclustbayesian.bug",
                              monitor=c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"),
                              data = thermalC3Wdata, inits = thermalC3Wini, 
                              n.chains = 2,
                              sample = 50000, burnin = 50000,
                              thin = 15, adapt = 40000,
                              method = "rjags", jags.refresh = 10)
...
###The posterior summary checks?
plot(SH3W_bayesian_spclust, "trace", vars = c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"))
plot(SH3W_bayesian_spclust, "autocorr", vars = c("beta", "sigma_y", "sigma.b","icc","tetha1", "tetha12", "phi", "sp_clus"))
dic_SH3W_spclust <- extract(SH3W_bayesian_spclust, what = "dic")
dic_SH3W_spclust
...
```{r}
knitr::kable(SH3W_bayesian_spclust$summaries[1:11, c(1:5)])
```

We observed that both Spatial model and Spatial cluster model with all the five clusters for Thermal Conductivity and Specfic Heat have lesser DIC  compared to models without cluster 4 and 5 respectively.
